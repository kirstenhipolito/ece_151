...
int ctr,ctr2;		//flag if data is sent 6 times
int seqnum1=0;
int seqnum2=0;
int srpacket1,srpacket2;
...
int main(int argc, char* argv[]) {
	...
	if (strcmp(argv[4],"sr") != 0 && strcmp(argv[4],"gbn") != 0){ //check if protocol input is gbn or sr
		perror("Protocol input is wrong!");
		exit(EXIT_FAILURE);
	}
	
	printf("The protocol you're trying to execute is %s\n", argv[4]);
	...
	n=1;
	ctr=0;
	if(strcmp(argv[4],"gbn") == 0){ //check if protocol is gbn
	printf("RUNNING GBN PROTOCOL\n");
	while (!(feof(fp)))
	{
		//send DATA to server
	if (n=1){ //in slow mode
		...
		SLOWMODE:	
		//check if error is due to timeout or due to error in transmitting when window size=2
		//if string length is zero, error is due to timeout
		...
		while(1){
			...
			while((segrecv->head.type)!=(ACK)) {
				if(sentSEQ1!=checkACK){ //CHECK if seqnum of sent and recv are the same
					//sentseq0 is the seqnum of the previous packet
					//if sentseq0=checkack, packet1buffer is lost
	...
	else if(strcmp(argv[4],"sr") == 0){ //check if protocol is sr
	printf("RUNNING SR PROTOCOL\n");
	while (!(feof(fp)))
	{
	//send DATA to server
	if (n=1){ //in slow mode
	...
	if(n=2){ //in SR
		...
		while(1){
			...
			if(srpacket1==0){			//resend if ACK is not recn sr packet1 is the ACK flag
			//sending first packet
			segsend1 = segment_populate(segsend1, DATA, seqnum1, packet1buffer);	//populate segment to be sent as a DATA segment
			printf("Send DATA Segment 1: %d, %d, %d, %s, %d\n", segsend1->head.type, segsend1->head.seqnum, segsend1->head.checksum, segsend1->data, segsend1->segsize);
			sentSEQ1=segsend1->head.seqnum;
			...
			ctr++;		//flag if data 1 is sent 6 times
			if(ctr>=6){
			goto TERMINATE;
			}
			}//srpacket1
			...
			ctr2++;		//flag if data 2 is sent 6 times
			if(ctr2>=6){
			goto TERMINATE;
			}
			}//srpacket2
			...
			while((segrecv->head.type)!=(ACK)) {
				if(sentSEQ1!=checkACK){ //CHECK if seqnum of sent and recv are the same
				...
				else{	//ACK 1 is now received
					srpacket1=1; //raise ACK1 flag so that packet 1 won't resend
					sentSEQ0=sentSEQ1;
					for (i=0;i<DATALENGTH;i++) {
						packet1buffer[i]=packet2buffer[i];
					}
					...
					else	//ACK2 is received
					{
						srpacket2=1; //raise ACK2 flag so that packet 2 won't resend
						break;
					}
				}//if sentseq1 ==checkack
				}//while not ack
				printf("Got ACK 1 %d and ACK 2 %d\n",checkACK,checkACK2);
				srpacket1=0;//clearACK1 flag
				srpacket2=0;//clearACK2 flag
				memset(segrecv, 0, sizeof(*segrecv));
				seqnuminc++;
				alarm(0);	//turn of the alarm because ACK is received
				printf("ALARM ENDS.\n");
